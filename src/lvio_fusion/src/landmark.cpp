...
Vector3d Landmark::GetPosition()
{
    return Camera::Get(1)->Pixel2Robot(cv2eigen(first_observation->keypoint), depth);
}

Vector3d Landmark::ToWorld()
{
    return Camera::Get()->Robot2World(GetPosition(), FirstFrame().lock()->pose);
}

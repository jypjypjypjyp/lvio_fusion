...
bool Initializer::EstimateVelAndRwg(std::vector<Frame::Ptr> Key_frames)
{
    if (!initialized)
    {
...
bool Initializer::Initialize(Frames keyframes, double priorA, double priorG)
{
    ...
    if (priorA == 0)
    {
        isOK = imu::InertialOptimization(keyframes, Rwg, 1e2, 1e4, isOptRwg);
    }
    else
    {
        isOK = imu::InertialOptimization(keyframes, Rwg, priorG, priorA, isOptRwg);
    }
    if (!isOK)
    {
        return false;

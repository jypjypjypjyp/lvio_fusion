...
void  Initializer::InitializeIMU(bool bFIBA)
{
    ...
    // imu计算初始时间
    mFirstTs=key_frames.front()->time;
    ...
    if (!initialized)
    {
        ...
        for(std::vector<Frame::Ptr>::iterator iter_frame = key_frames.begin()+1; iter_frame!=key_frames.end(); iter_frame++) 
        {
            if (!(*iter_frame)->preintegration->isPreintegrated)
            {
                continue;                
            }
            if(!(*iter_frame)->last_keyframe)
            {
                dirG -= (*iter_frame)->last_keyframe->GetImuRotation()*(*iter_frame)->preintegration->GetUpdatedDeltaVelocity();
            }
            if(!(*iter_frame)->last_keyframe->last_keyframe)
            {
                continue;
            }
            Vector3d velocity = ((*iter_frame)->GetImuPosition() - (*(iter_frame))->last_keyframe->last_keyframe->GetImuPosition())/((*iter_frame)->last_keyframe->preintegration->dT+ (*(iter_frame))->last_keyframe->last_keyframe->preintegration->dT);
            (*iter_frame)->last_keyframe->SetVelocity(velocity);
        }
        dirG = dirG/dirG.norm();
        ...
        Frame::Ptr current_frame=(*(--key_frames.end()));//最后一帧
        Vector3d Gz ;
        Gz << 0, 0, -9.81007;
        Gz =Rwg*Gz;

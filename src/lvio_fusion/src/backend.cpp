    ...
    for (auto kf_pair : active_kfs)
    {
        ...
        for (auto feature_pair : frame->left_features)
        {
            ...
            else if (first_frame != frame)
            {
                ...
                cost_function = TwoFrameReprojectionError::Create(init_ob->keypoint, feature->keypoint, landmark->depth, left_camera_);
...
{
    Map::Keyframes active_kfs = map_->GetActiveKeyFrames(full ? 0 : ActiveTime());
    ...
    options.max_solver_time_in_seconds = range_;
    ...
    for (auto kf_pair : active_kfs)
    {
        ...
        for (auto feature_pair : left_features)
        {
            ...
            auto first_frame = landmark->FindFirstFrame();
            Vector2d error(0, 0);
            if (first_frame->time < active_kfs.begin()->first)
            {
            ...
            else if (first_frame != frame)
            {
                double *para_fist_kf = first_frame->pose.data();
                auto init_ob = landmark->observations.begin()->second;
                TwoFrameReprojectionError(init_ob->keypoint, feature->keypoint, landmark->depth, left_camera_)(para_fist_kf, para_kf, error.data());
            }
            if (error.norm() > 3)
            {
...
void Backend::Propagate(double time)
{
    ...
    Map::Keyframes active_kfs = map_->GetActiveKeyFrames(time);

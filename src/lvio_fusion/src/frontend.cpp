...
void Frontend::PreintegrateIMU()
{
    ...
    std::vector<ImuPoint> imu_from_last_frame;
    while (true)
    {
        if (imu_buf.empty())
        {
        ...
        ImuPoint imudata = imu_buf.front();
        ...
        {
            imu_from_last_frame.push_back(imudata);
            imu_buf.pop_front();
        }
        else
        {
    ...
    if (!imu_preintegrated_from_last_frame->isBad)
        current_frame->preintegrationFrame = imu_preintegrated_from_last_frame;
}
//IMUEND
...
int Frontend::Relocate(Frame::Ptr base_frame)
{
    ...
    if (base_frame != last_keyframe)
    {
        ...
        imu_preintegrated_from_last_kf = current_frame->preintegrationFrame;
        //IMUEND
...
void Frontend::UpdateFrameIMU(const Bias &bias_)
{
    ...
    if (std::fabs(current_frame->time - current_frame->last_keyframe->time) > 0.001 && current_frame->preintegration && current_frame->last_keyframe)
    {
...
{
    if (Map::Instance().map_updated && last_keyframe)
    {
        Vector3d Gz;
        Gz << 0, 0, -Imu::Get()->G;
        Gz = Imu::Get()->Rwg * Gz;
        ...
        Vector3d twb2 = twb1 + Vwb1 * t12 + 0.5f * t12 * t12 * Gz + Rwb1 * current_frame->preintegration->GetDeltaPosition(last_keyframe->GetImuBias());
        Vector3d Vwb2 = Vwb1 + t12 * Gz + Rwb1 * current_frame->preintegration->GetDeltaVelocity(last_keyframe->GetImuBias());
        current_frame->SetVelocity(Vwb2);
        current_frame->SetPose(Rwb2, twb2);
        current_frame->SetNewBias(last_keyframe->GetImuBias());
        Map::Instance().map_updated = false; //IMU
    }
    else if (!Map::Instance().map_updated)
    {

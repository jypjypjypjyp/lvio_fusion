...
class ReprojectionError : public ceres::SizedCostFunction<2, 4, 3, 3>
{
public:
    ReprojectionError(Vector2d observation, Camera::Ptr camera)
                          ...
                          double **jacobians) const
    {
        ...
        if (jacobians)
        {
            ...
            if (jacobians[0])
            {
                ...
                Eigen::Matrix<double, 3, 4> jaco_Pc_2_Pose;
            ...
            if (jacobians[1])
            {
                ...
                Eigen::Matrix<double, 3, 3> jaco_Pc_2_Pose;
                jaco_Pc_2_Pose.setZero();
                jaco_Pc_2_Pose.block<3, 3>(0, 0) = Eigen::Matrix3d::Identity();
                jacobian = sqrt_information_ * jaco_res_2_Pc * jaco_Pc_2_Pose;
            }
            if (jacobians[2])
            {

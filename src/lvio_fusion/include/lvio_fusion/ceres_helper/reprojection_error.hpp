...
{
public:
    ReprojectionError(Vector2d observation, Camera::Ptr camera) : observation_(observation), camera_(camera) {}
                          ...
                          double **jacobians) const
    {
        ...
        residual = sqrt_information * (observation_ - camera_->camera2pixel(P_c));
        ...
        if (jacobians)
        {
            ...
            if (jacobians[1])
            {
                ...
                jacobian = sqrt_information * jaco_res_2_Pc * R;
            }
        }

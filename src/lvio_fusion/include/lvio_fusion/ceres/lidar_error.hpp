...
{
public:
    LidarPlaneErrorRPZ(LidarPlaneError origin_error, SE3d Tcw1, double *rpyxyz)
    ...
    static ceres::CostFunction *Create(const Vector3d p, const Vector3d pa, const Vector3d pb, const Vector3d pc, const Lidar::Ptr lidar, const SE3d Tcw1, double *rpyxyz)
    {
        LidarPlaneError origin_error(p, pa, pb, pc, lidar);
        return (new ceres::AutoDiffCostFunction<LidarPlaneErrorRPZ, 1, 1, 1, 1>(new LidarPlaneErrorRPZ(origin_error, Tcw1, rpyxyz)));
    }
...
{
public:
    LidarPlaneErrorYXY(LidarPlaneError origin_error, SE3d Tcw1, double *rpyxyz)
        : origin_error_(origin_error), Tcw1_(Tcw1), rpyxyz_(rpyxyz) {}
    ...
    {
        LidarPlaneError origin_error(p, pa, pb, pc, lidar);
        return (new ceres::AutoDiffCostFunction<LidarPlaneErrorYXY, 1, 1, 1, 1>(new LidarPlaneErrorYXY(origin_error, Tcw1, rpyxyz)));
    }

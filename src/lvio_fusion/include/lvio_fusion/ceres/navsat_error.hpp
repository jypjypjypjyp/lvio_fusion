...
{
public:
    NavsatInitError(Vector3d p0, Vector3d p1, double *weights)
    ...
    bool operator()(const T *tf, T *residuals) const
    {
        ...
        residuals[0] = T(weights_[0]) * (T(x0_) - tf_p1[0]);
        residuals[1] = T(weights_[1]) * (T(y0_) - tf_p1[1]);
        residuals[2] = T(weights_[2]) * (T(z0_) - tf_p1[2]);
        return true;
    }

    static ceres::CostFunction *Create(Vector3d p0, Vector3d p1, double *weights)
    {
        return (new ceres::AutoDiffCostFunction<NavsatInitError, 3, 7>(new NavsatInitError(p0, p1, weights)));
    }

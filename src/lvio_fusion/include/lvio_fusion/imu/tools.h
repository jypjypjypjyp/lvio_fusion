...
namespace imu
{
    ...
    bool InertialOptimization(Frames &key_frames, Eigen::Matrix3d &Rwg,double priorG, double priorA,bool isOptRwg);
    
    void FullInertialBA(Frames &key_frames, double priorG, double priorA);
    
    void FullInertialBA(Frames &key_frames);
    
    void RecoverData(Frames active_kfs,SE3d old_pose,bool bias=true);
}
